...
class TapPad
  ...
  moveAtom: (atom) ->
    ...
    if not @isValidMove nextX, nextY
      ...
      if atom.vertical
        @player.play "audio#{atom.x}"
      else
        @player.play "audio#{atom.y}"
    
    #move the atom and save it's position in the grid
    atom.move()

    @grid[atom.y][atom.x]["#{atom.id}"] = atom
  ...
  purgeOldAtoms: () ->
    keep = []
    for atom in @atoms
      if atom.collisions > @collisionsLimit or atom.moves > @movesLimit
        delete @grid[atom.y][atom.x]["#{atom.id}"]
      else
        keep.push atom
    @atoms = keep
